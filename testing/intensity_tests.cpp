...
protected:
  ...
  static constexpr const uint16_t relPowLow{
      54}; // 162 = 54% of 300, upper edge of P1
...
TEST_F(IntensityTest, PowerTests) {
  m_intensity = std::make_unique<Intensity>(powerLow, IntensityUnit::Watts, ftp,
                                            Level::Low);
...
TEST_F(IntensityTest, PercentFTPTests) {
  m_intensity =
      std::make_unique<Intensity>(powerPair, IntensityUnit::PercentFTP, ftp);
...
TEST_F(IntensityTest, AbsoluteFromPercentFTPTests) {
  m_intensity = std::make_unique<Intensity>(IntensityPair{powerLow, powerHigh},
...
TEST_F(IntensityTest, PowerZoneFromPercentFTPTests) {
  m_intensity = std::make_unique<Intensity>(IntensityPair{zero, highRelPower},
...
TEST_F(IntensityTest, HeartRateTests) {
  m_intensity = std::make_unique<Intensity>(hrLow, IntensityUnit::HeartRateBPM,
                                            maxHR, Level::Low);
...
TEST_F(IntensityTest, GetHeartRateBPMTests) {
  m_intensity =
      std::make_unique<Intensity>(hrPair, IntensityUnit::HeartRateBPM, maxHR);
...
TEST_F(IntensityTest, GetHRPercentMaxHRTests) {
  m_intensity = std::make_unique<Intensity>(IntensityPair{lowRelHR, highRelHR},
...
TEST_F(IntensityTest, HRZoneFromPercentMaxHRTests) {
  ...
  // Check absolute Values to Zones
  m_intensity = std::make_unique<Intensity>(IntensityPair{hrLow, hrHigh},
...
TEST_F(IntensityTest, HRZoneBoundaryTests) {
  // H1
  m_intensity = std::make_unique<Intensity>(IntensityPair{50, 60},
                                            IntensityUnit::PercentMaxHR, maxHR);
  if (auto retVal{m_intensity->getHeartRateZone(Level::Low)}; retVal) {
  ...
  // H2
  m_intensity = std::make_unique<Intensity>(IntensityPair{61, 70},
                                            IntensityUnit::PercentMaxHR, maxHR);
  if (auto retVal{m_intensity->getHeartRateZone(Level::Low)}; retVal) {
  ...
  // H3
  m_intensity = std::make_unique<Intensity>(IntensityPair{71, 80},
                                            IntensityUnit::PercentMaxHR, maxHR);
  if (auto retVal{m_intensity->getHeartRateZone(Level::Low)}; retVal) {
  ...
  // H4
  m_intensity = std::make_unique<Intensity>(IntensityPair{81, 90},
                                            IntensityUnit::PercentMaxHR, maxHR);
  if (auto retVal{m_intensity->getHeartRateZone(Level::Low)}; retVal) {
    EXPECT_EQ(*retVal, HRZ::H4);
  } else {
  ...
  // H5
  m_intensity = std::make_unique<Intensity>(IntensityPair{91, 100},
                                            IntensityUnit::PercentMaxHR, maxHR);
  if (auto retVal{m_intensity->getHeartRateZone(Level::Low)}; retVal) {
...
TEST(EqualTests, IntensityEqualTest) {
  Intensity ref{IntensityPair{1, 1}, IntensityUnit::Watts, 1};
  Intensity eq{ref};
  Intensity neqIntensity{IntensityPair{1, 2}, IntensityUnit::Watts, 1};
  Intensity neqUnits{IntensityPair{1, 1}, IntensityUnit::PercentFTP, 1};
  Intensity neqCap{IntensityPair{1, 1}, IntensityUnit::Watts, 2};
  EXPECT_TRUE(ref == eq);
  EXPECT_FALSE(ref == neqIntensity);
  EXPECT_FALSE(ref == neqUnits);
  EXPECT_TRUE(ref != neqCap);
}